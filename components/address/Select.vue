...
<template>
  <div btn-transparent md:btn-default px-3 @click="ui.addressDialogOpen = true">
    <!-- <Icon v-if="!ui.mobile" name="dropdown" text-xs text-dim mr-2 /> -->
    <template v-if="activeAddress">
      <AddressIcon :address="activeAddress" flex w-5 h-5 rounded-lg border-solid border-zinc-400 />
      <AddressDisplay :address="activeAddress" ml-2 mr-0.5 lt-md:hidden />
    </template>
    <template v-else>
